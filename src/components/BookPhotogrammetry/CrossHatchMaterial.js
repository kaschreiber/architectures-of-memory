...
class CrossHatchMaterial extends MeshStandardMaterial {
  constructor(options) {
    ...
    const paperTexture = loader.load("/assets/Watercolor.jpg");
    ...
    this.onBeforeCompile = (shader, renderer) => {
      ...
      shader.fragmentShader = shader.fragmentShader.replace(
        ...
        float blendDarkenFloat(float base, float blend) {
          return min(blend, base);
        }
        
        vec3 blendDarkenBasic(vec3 base, vec3 blend) {
          return vec3(
            blendDarkenFloat(base.r, blend.r),
            blendDarkenFloat(base.g, blend.g),
            blendDarkenFloat(base.b, blend.b)
          );
        }
        
        vec3 blendDarken(vec3 base, vec3 blend, float opacity) {
          return blendDarkenBasic(base, blend) * opacity + base * (1.0 - opacity);
        }
...
// Generiere die GUI-Parameter nur einmal
function generateParams(gui, material) {
  if (!gui) gui = new GUI(); // Erstelle GUI nur, wenn es noch nicht existiert
  ...
  gui
    .add(params, "e", 0, 1, 0.01)
    .onChange((v) => (material.uniforms.e.value = v));

  return gui; // Rückgabe des GUI-Objekts
}
